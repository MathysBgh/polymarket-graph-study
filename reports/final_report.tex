...
\begin{table}[t]
\caption{Schema mapping from the upstream SQLite tables to the canonical pipeline schema. Direct copies are flagged \emph{rename}; the spread is a derived no-arbitrage gap; the spot anchor is matched on the snapshot hour.}
\label{tab:schema}
\begin{center}
\small
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Canonical column}   & \textbf{Source}                                              & \textbf{Definition} \\
\midrule
\multicolumn{3}{@{}l}{\emph{markets}} \\
\texttt{market\_id}         & \texttt{markets.condition\_id}                                & rename \\
\texttt{asset}              & \texttt{markets.asset}                                        & uppercased \\
\texttt{contract\_type}     & \texttt{markets.direction}                                    & lowercased \\
\texttt{strike}             & \texttt{markets.threshold}                                    & rename \\
\texttt{settlement\_time}   & \texttt{markets.settlement\_date}                             & parsed as UTC \\
\texttt{label}              & \texttt{markets.outcome}                                      & rename \\
\midrule
\multicolumn{3}{@{}l}{\emph{observations}} \\
\texttt{market\_id}         & \texttt{market\_prices.condition\_id}                         & rename \\
\texttt{timestamp}          & \texttt{market\_prices.timestamp}                             & seconds, parsed as UTC \\
\texttt{mid\_price}         & \texttt{market\_prices.yes\_price}                            & direct \\
\texttt{spread}             & $|1 - \texttt{yes\_price} - \texttt{no\_price}|$               & no-arbitrage gap \\
\texttt{liquidity}          & \texttt{market\_prices.volume}                                & direct \\
\texttt{volume}             & \texttt{market\_prices.trade\_count}                          & direct \\
\texttt{reference\_spot\_price} & \texttt{ohlcv.close}                                       & matched on snapshot hour \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
